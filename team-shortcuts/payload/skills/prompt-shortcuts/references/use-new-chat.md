...
[คำถามเลือกรูปแบบการใช้ AI — ต้องถามและรอคำตอบก่อนวิเคราะห์งาน]
หลังแสดง New Chat Startup Report ให้แสดงปุ่ม 1/2 ถ้าหน้าจอรองรับ หรือให้ผู้ใช้พิมพ์เลข 1/2:
...
ห้ามเรียก AI เพื่อวิเคราะห์งาน ห้ามสร้างแผน และห้ามเริ่มงานเขียนจนกว่าผู้ใช้เลือก 1 หรือ 2 · เมื่อเลือกแล้วให้ส่งค่าต่อไปยัง AI Relay โดยไม่ถามซ้ำในแชทเดียวกัน
...
[ด่านก่อนเขียนทุกงาน — บังคับซ้ำ ไม่ใช่ตรวจครั้งเดียวตอนเปิดแชต]
- การอนุมัติ branch ใช้ได้กับ `task_id + ขอบเขต + รายการ path` หนึ่งชุดเท่านั้น · คำสั่งผู้ใช้ก้อนใหม่ การเปลี่ยนหัวข้อ/ขอบเขต การกลับมาหลังคำถามแทรก หรือหลังย่อบริบท = ต้องตรวจใหม่เสมอ
...
- จัดประเภททุกงานเป็น `CONTINUATION` หรือ `NEW_WRITABLE_TASK` พร้อมหลักฐาน:
  ...
  - ไม่ครบข้อใดข้อหนึ่ง = `NEW_WRITABLE_TASK` · ต้องเสนอ branch ใหม่ใน registered folder เดิมและรอเจ้าของอนุมัติก่อนสร้าง/เขียน
- คำว่า “ทำต่อ”, “แก้เพิ่ม”, “อีกเรื่อง”, หรือการอยู่ในแชตเดิม ไม่ใช่หลักฐานว่าเป็นงานต่อเดิม
- ก่อนเขียนให้ประกาศ `Write Permit`: task_id / branch / base SHA / allowed paths / owner approval / claim status · ไม่มีหรือค่าเปลี่ยน = ห้ามเขียน
...
- ห้ามเสนอ ห้ามสร้าง และห้ามสั่งสร้าง Git worktree ใหม่ ไม่ว่างานใหม่ งานขนาน พื้นที่ dirty หรือ AI ตัวใดร้องขอ
- ถ้า registered folder ไม่มี = `MISSING_REGISTERED_WORKSPACE` · หยุดและรายงานเจ้าของ ห้าม fallback ไป main/คนอื่นหรือสร้างใหม่
- งานใหม่สร้างได้เฉพาะ branch ภายใน registered folder เดิม หลังยืนยันว่าพื้นที่สะอาดและเจ้าของอนุมัติ
- หลาย Cursor/AI ใช้โฟลเดอร์เดียวกันได้ แต่มีงานเขียนพร้อมกันได้ 1 งาน; งานอื่นอ่านอย่างเดียวหรือรอคิว AI Relay
...
- dirty worktree → STOP · ห้ามย้าย changes, stash หรือสร้าง worktree ใหม่ · รายงานว่าเป็นงานใครและปิด/ส่งต่อก่อน
สร้าง branch ใหม่ใน registered folder เดิมเท่านั้น: ใช้มาตรฐานชื่อ branch ของโปรเจกต์; ถ้าไม่มี ใช้ทีม/VPS = `team/<staff>/<slug>` · local = `feature/<slug>` (มี ticket id) · ชื่อชน = เสนอชื่อใหม่ ไม่ทับ · เสนอ+ขอยืนยันก่อนสร้าง
