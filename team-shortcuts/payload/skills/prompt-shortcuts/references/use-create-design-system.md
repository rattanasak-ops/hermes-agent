...
# Use Create Design System / Use Design System (v3.1 · 2026-07-16)
...
>
> หน้าที่: พิมพ์คำสั่งนี้ในโปรเจกต์ไหนก็ได้ แล้ว AI จะ (0) ถามก่อนว่าจะทำ Design System สำหรับ "หน้าเว็บ (Front)" หรือ "ระบบหลังบ้าน (Application)" (1) อ่านเป้าหมาย/แผนธุรกิจ → เสนอบุคลิกดีไซน์โดยอิงดีไซน์ระดับโลกตามโหมดที่เลือก (2) ค้นของเดิม + ประเมินช่องว่าง (3) ด่านสี: มีสีเดิม → เสนอยืนยันชุดเดิม (เจ้าของยืนยันเสมอ) · ไม่มี → หยุดถามเจ้าของเรื่องสีแบรนด์ 4 สี + น้ำหนัก แล้วต่อยอดเป็นชุดสีครบ (4) สร้างไฟล์มาตรฐาน + ปรับโปรเจกต์ให้ตรงมาตรฐานแบบไม่พังของเดิม
>
...
[กฎกันกลืนเป้าหมายแม่ — บังคับก่อนสวมบท (F-02 · บทเรียนจริง 2026-07-16 AI หลงเป้ารอบที่ 20)]
- บทข้างบนเป็นบท "ระหว่างทำงานดีไซน์" เท่านั้น ห้ามให้มันลบเป้าหมายของคนที่เรียกใช้คำสั่งนี้
- ก่อนเริ่ม ให้จดว่า "ใครเรียกคำสั่งนี้ เพื่ออะไร" (เช่น ทำ DS ให้โปรเจกต์จริง / ทดลองนำร่องเพื่อพิสูจน์ตัวคำสั่ง / ตรวจย้อนของเดิม) แล้ว**ทุกข้อความตอบต้องขึ้นต้นด้วยบรรทัดประกาศสถานี**:
  `[งานแม่: <ใครเรียก·เพื่ออะไร> · ขั้นนี้: <เฟสของคำสั่งนี้> · พิสูจน์: <สิ่งที่ขั้นนี้ต้องพิสูจน์>]`
- โหมดทดลองนำร่อง (pilot): ถ้าถูกเรียกเพื่อทดสอบ/แก้ตัวคำสั่งนี้เอง สิ่งส่งมอบหลักคือ**บัญชีข้อบกพร่องของคำสั่ง** (ทุกขั้นจบด้วย "ขั้นนี้เจอจุดอ่อนอะไรของคำสั่ง") — DS ของโปรเจกต์นำร่องเป็นผลพลอยได้ ไม่ใช่ตัวงาน
- กันพึ่งความจำแชท: **จดเป้าหมายแม่แบบคำต่อคำลงหัวไฟล์ `.project/DesignSystem.md` (บรรทัด `> caller_goal: ...`) ตั้งแต่ก้าวแรก** · บรรทัดประกาศสถานีให้คัดจากช่องนี้เสมอ ไม่ใช่เรียบเรียงใหม่จากความจำ
- ขาดบรรทัดประกาศสถานี = ถือว่าเริ่มหลงเป้า ให้หยุดแล้วเปิด `caller_goal` ใน `.project/DesignSystem.md` ทวนก่อนตอบต่อ
...
- [ด่านตรวจรุ่นสำเนา — F-01 · บังคับ · กัน version drift ที่ปลายทาง] ถ้าโปรเจกต์มีสำเนา `design-system-standard-v2/` ของตัวเอง ต้องเทียบกับชุดกลางก่อนใช้ทุกครั้ง — **เทียบเนื้อจริงทั้งชุด ไม่ใช่แค่เลขรุ่น** (เช่น `git diff --no-index <ชุดกลาง> <สำเนาโปรเจกต์>` หรือเทียบค่าแฮชรายไฟล์ ยกเว้น `_ledger/`) เพื่อจับทั้งสำเนาเก่า สำเนาขาดไฟล์ และสำเนาที่ถูกแก้บางไฟล์ · พบไม่ตรง = **เสนอ "อัปเดตสำเนาให้ตรงชุดกลาง" เป็นงานชิ้นแรกของแผนให้เจ้าของอนุมัติ** (ตามกฎเหล็กข้อ 1 ห้ามแก้ไฟล์ก่อนอนุมัติแผน — ถ้าเจ้าของอนุมัติเฟสงานครอบคลุมแล้วเดินได้เลยผ่านสายพานเขียนของโปรเจกต์) · ห้ามเดินเฟสถัดไปด้วยสำเนาที่ไม่ตรง — เคสจริง: Root Admin ถือสำเนา v2 (77 ข้อ ไม่มี ds-gate) ทั้งที่ชุดกลางเป็น v3.1 มาแล้ว
...
- [ทางลัดโปรเจกต์มีสีเดิม — F-03] ก่อนถามสี ให้จำแนกสีเดิมของโปรเจกต์เป็น 1 ใน 3 แบบ พร้อมหลักฐาน: (ก) **อนุมัติแล้ว** — มีบันทึกเจ้าของ/กรรมการอนุมัติ (decision/มติ/เอกสารแบรนด์) (ข) **ใช้งานอยู่แต่ไม่พบหลักฐานอนุมัติ** — มี token/theme ในโค้ดจริง (ค) **ไม่มีสีเดิม** · แบบ (ก)(ข): **ห้ามถามหา 4 สีใหม่จากศูนย์** — สกัดชุดเดิมมาแสดงพร้อมป้ายจำแนก+เหตุผล แล้วเสนอค่าแนะนำ 1 คำตอบ "ยืนยันชุดเดิม + น้ำหนักที่แนะนำ" · แบบ (ข) ต้องบอกชัดว่า "ยังไม่พบหลักฐานว่าเคยอนุมัติ" ให้เจ้าของรู้ก่อนยืนยัน — กันรับรองสีเก่าที่ผิดโดยไม่ตั้งใจ · **ทุกแบบเจ้าของยังเป็นคนยืนยันชุดสีเสมอ** · ถามแบบเต็ม 4 สีเฉพาะแบบ (ค) หรือเจ้าของสั่งรื้อสีเอง
- ถามเจ้าของ (กรณีไม่มีสีเดิม): "สีแบรนด์หลัก 4 สี จะใช้สีอะไร และน้ำหนัก/สัดส่วนเท่าไหร่" (เช่น สีหลัก/สีรอง/สีเน้น/สีกลาง)
...
- v3.1 (2026-07-16): ปิดข้อบกพร่อง 3 ข้อแรกจากการทดลองนำร่อง Root Admin (บัญชีเต็ม: repo Hermes Agent `.project/dsu-pilot-findings.md`) — **F-01** ด่านตรวจสำเนามาตรฐานในโปรเจกต์ก่อนใช้: เทียบเนื้อจริงทั้งชุดกับชุดกลาง ไม่ใช่แค่เลขรุ่น + การอัปเดตต้องเข้าแผนให้เจ้าของอนุมัติ (กัน version drift ปลายทาง · เคสจริง: สำเนา v2 ค้างใน synerry-engine) · **F-02** กฎกันกลืนเป้าหมายแม่: จด `caller_goal` คำต่อคำลง DesignSystem.md + บังคับบรรทัดประกาศสถานีทุกข้อความ + โหมด pilot ที่สิ่งส่งมอบ = บัญชีข้อบกพร่องของคำสั่ง (เคสจริง: AI หลงเป้ารอบที่ 20 เพราะบท Engineer กลืนเป้าหมาย) · **F-03** ด่านสีจำแนกสีเดิม 3 แบบ (อนุมัติแล้ว/ใช้อยู่ไม่มีหลักฐานอนุมัติ/ไม่มี) + ทางลัด "ยืนยันชุดเดิม" โดยเจ้าของยังยืนยันเสมอ ห้ามบังคับถาม 4 สีจากศูนย์ · ผ่านผู้ตรวจต่างค่าย (GPT-5 · fix-then-proceed → แก้ครบ 3 ช่อง: แฮชสำเนา/caller_goal/จำแนกสี+แก้บทนำให้ตรงกัน)
