...
## DEC-AIR-001 · AI ทุกตัวต้องผ่าน AI Portal และสิทธิ์กลาง (2026-07-18 · เจ้าของยืนยัน 5/5 ข้อ)

Decision: Cursor, Claude Code App และ Codex App ต้องส่งงาน AI ผ่าน AI Portal โดยใช้กฎผู้ใช้/บทบาทชุดเดียวกัน · Fable ต้องเปิดเป็นโมเดลใน Portal และ Portal เป็นผู้ตรวจสิทธิ์ · Grok มีบัญชีอยู่แล้ว ปัญหาที่ต้องแก้คือการเชื่อมต่อหลุด · Codex มี 2 ID และ Portal ต้องเลือก ID ทำงานที่ตรงข้ามกับ ID ที่กำลังล็อกอิน

ผลแทนที่: ข้อความ `DEC-FBL-001` ในสาขาเก่า `relay-fable-memory` ที่ว่าเครื่องเจ้าของใช้ Fable ผ่าน Claude Subscription CLI ตรง **เลิกใช้ตั้งแต่ 2026-07-18 และไม่นำเข้ามาใน main** · หลักฐานการรันวันที่ 16 ก.ค. ยังเป็นประวัติศาสตร์ที่เกิดขึ้นจริง แต่ห้ามใช้เป็นแบบการเชื่อมต่อปัจจุบัน · Relay ห้ามเปลี่ยนไป CLI ในเครื่องหรือเปลี่ยนยี่ห้อ AI แบบเงียบเมื่อ Portal ใช้ไม่ได้

Reason:

- ให้สิทธิ์ งบ บัญชี และร่องรอยการใช้งานตัดสินจากจุดเดียว
- ทำให้โปรแกรมทั้ง 3 ตัวให้ผลสิทธิ์ตรงกันและตรวจย้อนหลังได้
- ป้องกันข้อสรุปผิดว่า Grok ไม่มีบัญชี ทั้งที่คำสั่งหลุดไปเรียก CLI คนละตัวบนเครื่อง
- ป้องกัน Codex ใช้ ID เดียวกับโปรแกรมที่เจ้าของกำลังล็อกอิน

หลักฐานและงานต่อ: ดู `.project/session-log-2026-07-18-ai-portal-routing.md` · ต้องตรึงสัญญา Portal-only ก่อนแก้ Relay และต้องทดสอบเส้นจริงก่อนกล่าวว่าพร้อมใช้
...
## 2026-07-16 — ชุด Use Migrate รายเฟส + การกระจาย (แชท Fable · เจ้าของอนุมัติในแชท)

- **DEC-MW-008**: แตก `Use Migrate Web` เป็นชุดรายเฟส `Use Migrate 0-13` + สัญญากลาง — เจ้าของถือกุญแจผ่านเฟสคนเดียว (พิมพ์เลขถัดไปเอง) · โครง 5 ส่วนบังคับ · จุดหยุดใน flow ชนะกฎ "ทำจบรอบเดียว" · เหตุ: root cause R1-R11 (AI เห็นคำสั่งทุกขั้นพร้อมกัน + FW-P0 ถูกข้าม + M5 hardcode 5%)
- **DEC-MW-009**: เฟสด่านเครื่อง (9/10/11/13) ไม่มีเครื่องมือ = `BLOCKED_TOOLING` จบเฟสไม่ได้ · ห้ามแทนด้วยภาพ+grep (กฎ R-3) · การให้ AI อื่นรีวิว ≠ การรันด่าน
- **DEC-MW-010**: โควตาเมนู — ค่าเริ่มต้น 1 คน = 1 เมนู · เกินได้เมื่อ owner อนุมัติ (`quota: <คน>=<N>` ใน menu-queue) · เฟสโค้ด 8-9 ทีละเมนูต่อพื้นที่เสมอ (โฟลเดอร์เดียวสลับกิ่งพร้อมกันไม่ได้)
- **DEC-VAULT-002**: กระจกคลัง Obsidian บน VPS = พื้นที่ทีมใช้ร่วม (มีไฟล์พนักงาน + สมุด git แยกเล่ม) — ห้ามยกทั้งโฟลเดอร์ · ซิงก์เฉพาะ `ai-context/` + `skills/prompt-shortcuts/` จากเครื่องเจ้าของ · งานแปลกบนนั้นกู้ขึ้นกิ่ง `vps-rescue-*` ใน GitLab ก่อนเสมอ · งาน rename `Use Request Merge` (28 มิ.ย.) เก็บกิ่ง rescue ไม่รวม main
- **DEC-STORE-001** (คำปรึกษา — เจ้าของรับ): มาตรฐานเก็บไฟล์อัปโหลดทุกโปรเจกต์ = โค้ดกลาง config-only (`STORAGE_BASE_PATH`) ห้ามแก้รายไซต์ · ไฟล์ = `uploads/{siteId}/{หมวด}/` นอก git · ชุดติดตั้งลูกค้า = โค้ดกลาง + ก้อน uploads ของ site + DB · โค้ดแก้ที่ Root Admin ครั้งเดียว · ข้อมูลเก่าตามเก็บรายโปรเจกต์ตอนทำ
- **แก้ความจำล้าสมัย**: "AI push คลัง Obsidian ไม่ได้" — push ขึ้น GitLab ได้จริงแล้ว (พิสูจน์ 2 ครั้ง 2026-07-16)

## 2026-07-16 — Flow Station Gate (แชทเช้า · PR #51 · บันทึกย้อนหลัง)

- **DEC-MW-011 · คำอนุมัติผ่านสถานีต้องยึดหลักฐานที่ AI แก้ไม่ได้ (2026-07-16)**
  - **ปฐมเหตุ:** เจ้าของเจอ AI ข้าม flow ซ้ำ 4 วัน — ด่าน owner_ok เดิมอยู่ในไฟล์ `.flow-state` ที่ AI เขียนเองได้ = AI ปลอมคำอนุมัติได้ (พิสูจน์แล้วว่าเกิดจริง)
  - **ทางแก้ (PR #51 · เจ้าของเลือกทาง ก):** station gate ใน `enforce-flow-gate.py` อ่านคำอนุมัติจาก Claude Code transcript (append-only) · นับเฉพาะข้อความคน (`origin.kind=human`) · สถานี+คำอนุมัติติดกัน ≤200 ตัวอักษร กัน paste ยาว · ข้ามประโยคปฏิเสธ/คำถาม · fail-closed เมื่ออ่าน transcript ไม่ได้ · คุมเฉพาะพื้นที่ MW
  - **บทเรียนต่อยอด DEC-MW-005:** สถานะ derive จากหลักฐานอย่างเดียวไม่พอ — **คำอนุมัติเจ้าของก็ต้อง anchor กับสิ่งที่ AI เขียนไม่ได้** (transcript จริง / merge SHA / ผลรันเครื่อง) · ไฟล์ state ใดที่ AI เขียนได้ = ใช้เป็นหลักฐานอนุมัติไม่ได้

## 2026-07-16 — ซ่อมยาม prewrite gate over-lock (แชท Fable · เจ้าของอนุมัติ "ok")

- **DEC-GATE-001 · prewrite gate ต้องคุมเฉพาะเขตที่ระบบจัดการ ไม่ล็อกทั้งเครื่อง**
  - ปฐมเหตุ: gate v1 fail-closed แบบกว้างเกิน (ทุก git repo + บล็อก git commit + บล็อกความจำ + ทะเบียนกลางล่มตายทั้งเครื่อง) → deadlock จนต้องถอด hook
  - กติกา: คุมเฉพาะ registered worktree roots + canonical ที่มี session อ้างถึง · repo อื่นทั้งเครื่องปล่อยผ่าน (ด่านอื่นดูแล) · ตัดสินขอบเขตจาก **ไฟล์เป้าหมาย** ไม่ใช่แค่ cwd (กันหลบด่านด้วย cd) · ทะเบียนกลางติดต่อไม่ได้/ตอบผิดรูป = ใช้ permit ท้องถิ่นที่ยังไม่หมดอายุ (WTL section 8) — infra พังต้องไม่แปลว่างานไม่พร้อม
- **DEC-GATE-002 · ด่านต้องกันการถอด/ปลอมตัวเอง (self-protection)**
  - GPT-5 ชี้: ถ้าปล่อยทุกเป้าหมายนอกเขตผ่าน AI จะเขียนทับเครื่องมือกลาง, ปลอม session, แก้ไฟล์ตั้งค่า, ลบ client hook = ถอดปลั๊กตัวเอง
  - กติกา: protected_target บล็อกการเขียน/rm พื้นที่โครงสร้างด่าน (โฟลเดอร์ new-chat tools, โฟลเดอร์ hook, ไฟล์ settings, เครื่องมือ hermes ใน local bin) เด็ดขาด ทั้ง write tool และ shell · ช่องความจำข้ามพื้นที่ = เฉพาะโปรเจกต์เดียวกัน
  - บทเรียน: ด่านความปลอดภัยทุกตัวต้องกันการแก้ตัวเองเป็นข้อแรก ไม่งั้น bypass ได้ในคำสั่งเดียว
- **DEC-GATE-003 · สายพาน relay ใช้ไม่ได้ = ใช้โหมด 2 (คนหลักเขียน + ต่างค่ายตรวจ + เครื่องตัดสิน)**
  - เครื่องนี้ relay-call ล้มทั้งสาย (Portal token ไม่มี + codex/gemini/ollama crash) → ทำตามกติกา Use AI Relay: Fable เขียน · GPT-5 (cross-check MCP) ตรวจต่างค่าย · ผู้ตรวจครบ 2 รอบเปลี่ยนเป็นเครื่องตัดสิน (pytest 89 + live hook 15) = verified ของจริง ไม่ใช่ปาก AI

## DEC-DSU-002 · Goal Lock — จบ Use Create Design System เท่านั้น ห้ามทำงานอื่น (2026-07-16)
- **🔴 เป้าหมายแม่ (สิ่งส่งมอบจริง — อ่านบรรทัดนี้ก่อนทุกบรรทัด):** งานนี้คือ **"แก้/อัปเดตตัว shortcut `Use Create Design System`"** ให้ใช้งานได้จริง — การ pilot กับโปรเจกต์ใด ๆ เป็นแค่ **เครื่องมือพิสูจน์ตัว shortcut** ไม่ใช่ตัวงาน · ทุกขั้นของ pilot ต้องจบด้วยคำถามเดียว: "ขั้นนี้พิสูจน์อะไรเกี่ยวกับ shortcut / เจอข้อบกพร่องอะไรของ shortcut ที่ต้องแก้" · ห้ามกลายร่างเป็น "คนทำ Design System ให้โปรเจกต์นำร่อง" แล้วลืมว่ากำลังทดสอบเครื่องมือ
- **กันหลงทางซ้ำ (บทเรียนรอบที่ 20 · 2026-07-16):** prompt ของ shortcut เขียนว่า "คุณคือ Senior Design System Engineer... ทำให้โปรเจกต์นี้มี design system" — พออ่านแล้ว AI จะถูกดูดเข้าบทนั้นและลืมเป้าหมายแม่ · ดังนั้นทุกข้อความรายงานในงานนี้ต้องขึ้นต้นด้วยบรรทัดประกาศสถานี: `[งานแม่: แก้ shortcut DSU · ขั้นนี้: <ชื่อขั้น> · พิสูจน์: <สิ่งที่ทดสอบ>]` (แก้ถาวรใน prompt v3.1 = กฎ F-02 แล้ว)
...
- **ผลรอบ 2026-07-16/17:** pilot Root Admin ครึ่งแรกผ่าน (`ds-gate` 20/20 exit 0 · สำเนามาตรฐานในโปรเจกต์อัปเป็น v3.1 commit `c669a7a4`) · บัญชีข้อบกพร่อง shortcut 5 ข้อ = `.project/dsu-pilot-findings.md` · แก้ F-01/F-02/F-03 เข้า prompt v3.1 (คลัง `e133f00` + ผู้ตรวจ GPT-5 + payload PR #58 merged `af3aa41db`) · F-04 ปิดด้วยการยืนยัน HEAD เลื่อนหลัง coder commit + regression test · F-05 ปิดด้วยการให้คำสั่งอ่านอย่างเดียวผ่าน gate + regression test 2/2

## DEC-SPEC-001 · สเปคกลางต่อ lifecycle แบบ capability-based (2026-07-17)
- **บริบท:** SPEC-CENTRAL — เพิ่มสเปคกลาง (`.project/spec/<plan_id>.md`) เข้าคำสั่งกลาง 6 ตัว หลัง PR #57 (พื้นฐาน) merged · ต่อยอดจากบทเรียน AI แต่ละตัวเข้าใจโจทย์ไม่ตรงกัน
- **การตัดสิน:** ทำแบบ **capability-based / additive ล้วน** ไม่แตะ version lock §8 — โปรเจกต์ไม่มีสเปคที่จับคู่แผน = เดินระบบเดิม 100% (กันพัง 30-40 โปรเจกต์เก่า)
- **กติกากลาง §1d (memory-schema):** จับคู่สเปคกับแผนด้วย **ชื่อไฟล์เท่านั้น** (มีแค่โฟลเดอร์ไม่นับ · กัน false-block) · `draft`=ห้ามโค้ด · `approved/building`=ทำได้ · `done`=ห้ามพฤติกรรมใหม่ · **`owner_approved` เปลี่ยนได้เมื่อเจ้าของพิมพ์อนุมัติเองเท่านั้น + จดหลักฐาน** (แยกจากอนุมัติแผน · AI ติ๊กเอง=ละเมิด DEC-040)
- **ที่มา:** รีวิวต่างค่าย GPT (partial-agree · fix-then-proceed) → ปิด 3 finding: กัน AI ติ๊กอนุมัติเอง / จับคู่ด้วยชื่อไฟล์ / done แคบลง
- **สถานะ:** P5 (คำสั่ง 6 ตัว + ทะเบียน 2 สำเนา) เสร็จ verified — branch `SPEC-P5-I1` SAFE_TO_MERGE · **S1 (ตัวบังคับจริงระดับโค้ด) ยังไม่ทำ** — สเปคตอนนี้เป็นด่านระดับคำสั่ง AI ยังปลอมได้ (แผน SPEC-P6/P7 ใน plan.md)
- coder=grok ผ่าน relay (codex rotate) โหมด 2 · ผู้ตรวจ=GPT cross-check

## DEC-FBL-001 · Fable 5 เฉพาะเครื่องเจ้าของ (2026-07-16 · PR #54/#55)
- Fable เปิดใช้ได้เฉพาะเครื่องที่มีไฟล์อนุญาต `~/.hermes/.fable-allowed` โดยมีตัวกั้นในโค้ดซ้ำอีกชั้น พนักงานและ VPS เรียกไม่ได้
- เครื่องเจ้าของเรียก Claude ผ่าน Subscription ตรง ไม่ใช่ AI Portal token
- การเรียก Claude ซ้อนต้องตัดตัวแปร `CLAUDECODE` ออกจากสภาพแวดล้อมของโปรแกรมลูก เพื่อป้องกันการล้มระหว่างทำงาน

## DEC-GATE-004 · แยกบันทึกปิดรอบจากกิ่งที่มีงานอื่นปน (2026-07-17 · PR #60)
- เมื่อกิ่งมีไฟล์งานจากหลายเซสชันปนกัน ให้รวมเฉพาะ commit ที่ต้องการผ่านพื้นที่สะอาด แล้วเปิด PR แยก ไม่แตะไฟล์ของเซสชันอื่น
- กิ่ง `dev` ใช้เป็นจุดส่งต่อทีมจาก `main` ล่าสุด และต้องตรวจให้เดินตาม `main` เมื่อมีการรวมงานใหม่

## DEC-BRM-002 · ปิด BRM ที่ขอบเขตส่งมอบ ไม่ลบประวัติของแชทอื่น (2026-07-19)
- `main` และ `origin/main` ตรงกันที่ `ae230bbd5ee55c18eb6a12f9dd6ae883fe67dc81` หลัง PR #80/#81 และสัญญาทดสอบแก้ไขรวมสำเร็จ; ไม่มี PR เปิดค้าง 0 รายการ
- ชุดทดสอบบนฐานล่าสุดผ่าน 174/174 และ 218/218; จึงนับ BRM-P5 ส่งงานเข้า `main` ครบ 3/3 = 100%
- Worktree/branch ประวัติ 18 รายการยังไม่ลบ เพราะมีงาน dirty หรือไม่มี owner/task ที่ยืนยัน; การเก็บไว้ลดความเสี่ยงข้อมูลของแชทอื่นสูญหาย และต้องทำ WTL แยกหากจะลบภายหลัง

## DEC-SHORTCUT-001 · AI ไม่มีสิทธิ์สร้าง Worktree เพื่อเดินงานปกติ (2026-07-19 · PR #84/#85)
- ต้นเหตุไม่ใช่พื้นที่ไม่พอ แต่เป็นคำสั่งขัดกันใน Shortcut/Skill, ตัวตรวจครอบคลุมไม่พอ และด่านนับประวัติหลัง squash ผิด
- AI ต้องใช้ Git root/กิ่งปัจจุบัน การสร้าง/สลับ/ย้าย/ลบ Worktree เป็นสิทธิ์ของเจ้าของหรือผู้จัดการพื้นที่เมื่อสั่งตรงเท่านั้น
- BLOCKED_DO_NOT_MERGE ต้องแก้จากประวัติและผลรวมจริง ห้ามหนีด้วย Worktree/กิ่งใหม่หรือการลดเพดาน

## DEC-SHORTCUT-002 · แผนกลางหนึ่งรหัสต่อไฟล์และ active หนึ่งแผน (2026-07-19 · PR #86)
- `.project/plan-index.md` เป็นดัชนีรวม; `.project/plan.md` เก็บแผน active เท่านั้น
- memory-audit บล็อกเมื่อมี active มากกว่าหนึ่ง, ไฟล์หนึ่งมีหลาย plan_id, พาธซ้ำ หรือรหัสไม่ตรงดัชนี
- สถานะปัจจุบัน: SHORTCUT 28/29 = 96.6%; เหลือ notebook ทีมที่ยังไม่มีช่องทางเข้าถึง
