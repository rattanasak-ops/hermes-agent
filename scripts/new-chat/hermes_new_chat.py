...
def open_task(args: argparse.Namespace) -> dict:
    ...
    command = [
        find_tool("hermes-worktree"), "open",
        "--project-id", args.project,
        "--staff-id", args.staff_id,
    ...
    permit = [
        find_tool("hermes-write-permit"), "acquire",
        "--cwd", str(worktree), "--task-id", args.task_id,
        "--branch", task["branch"], "--base-sha", base_sha,
        "--approval", args.approval,
        "--ttl-minutes", str(args.lease_hours * 60),
    ]
    for value in args.allowed_path:
        permit += ["--allowed-path", value]
    permit_data = run_json(permit, env)
    state = {
        "schema": "hermes-new-chat-v1",
        ...
        "canonical_repo": str(Path(args.repo).expanduser().resolve()),
...
def status_task(args: argparse.Namespace) -> dict:
    ...
    permit_expires_at = dt.datetime.fromisoformat(str(state.get("permit_expires_at", "")))
    if permit_expires_at.tzinfo is None:
        permit_expires_at = permit_expires_at.replace(tzinfo=dt.timezone.utc)
    permit_ready = dt.datetime.now(dt.timezone.utc) < permit_expires_at
    state["wtl"] = live["decision"]
    state["status"] = (
        "NEW_CHAT_READY"
        if live["decision"] == "WTL_READY" and permit_ready
        else "NEW_CHAT_BLOCKED"
    )
    return state
